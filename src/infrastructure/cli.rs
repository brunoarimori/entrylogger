...
impl CliInput {
  ...
  pub fn listen(&self) -> Result<EntryObject, String> {
    ...
    if matches.is_present("post") {
      let date = self.entry_business.validate_date(self.read_date().unwrap().as_str())?;
      let time = self.entry_business.validate_time(self.read_time().unwrap().as_str())?;
      let tag = self.entry_business.validate_tag(self.read_tag().unwrap().as_str())?;
      let message = self.entry_business.validate_message(self.read_message().unwrap().as_str())?;
